  ...
  return (
    <div className="group relative border-t-2 border-[#6E56CF]">
      ...
      {/* Open drivers: click state (tablet/mobile) below lg; pure hover on
          desktop (lg+), exactly like the original interaction. */}
      <div
        id={panelId}
        className={`mx-auto grid w-full max-w-3xl transition-[grid-template-rows] duration-500 ease-out lg:group-hover:grid-rows-[1fr] ${
          isOpen ? "grid-rows-[1fr]" : "grid-rows-[0fr]"
        }`}
      >
        <div className="overflow-hidden">
          <p className="px-6 pb-6 text-base md:text-lg text-neutral-700 leading-relaxed">
            {answer}
          </p>
        </div>
      </div>
...
export function FAQSection() {
  ...
  return (
    <section id="faq" className="relative z-10 py-24 md:py-32" style={{ backgroundColor: "#F8F9FB" }}>
      ...
      {/* ── Ambient mesh glow — dedicated background layer ─────────────────────
          ...
          cyan wash top-left, a faint violet wash bottom-right, each drifting
          ...
          edge into the neighbouring section — a little cyan into Testimonials
      ...
      <div className="relative z-10 mx-auto flex max-w-4xl flex-col items-center px-6 text-center">
        ...
        <SplitText as="div" reveal revealDelay={0.15} className="w-full border-b-2 border-[#6E56CF]">
          {faqs.map((faq, i) => (
            <FAQItem
              key={i}
