...
export const LogoLoop = memo(({ logos, speed = 120, direction = 'left', width = '100%', logoHeight = 40, gap = 32, pauseOnHover, hoverSpeed, fadeOut = false, className, style }) => {
  ...
  const rootClassName = useMemo(() => ['logoloop', isVertical ? 'logoloop--vertical' : 'logoloop--horizontal', fadeOut && 'logoloop--fade', className].filter(Boolean).join(' '), [isVertical, fadeOut, className]);
  ...
  const renderLogoItem = useCallback((item, key) => {
    return (
      <li className="logoloop__item" key={key}>
        <span className="logoloop__node flex items-center justify-center">{item.node}</span>
      </li>
    );
  }, []);
  ...
  const containerStyle = useMemo(() => ({ width: toCssLength(width) ?? '100%', ...cssVariables, ...style }), [width, cssVariables, style, isVertical]);
